...
function buildItems(pool, seg) {
  ...
  if (pool.length > totalSlots) {
    console.warn(
      `[DomeGallery] Provided image count (${pool.length}) exceeds tiles (${totalSlots}). Some images will not be shown.`
    );
  }
...
export default function DomeGallery({
  ...
  // Lazy reveal hook (evita di scaricare subito tutte le immagini)
  useRevealImages();
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      root.style.setProperty("--gradient-top", topGradientColor);
      root.style.setProperty("--gradient-bottom", bottomGradientColor);
        ...
        >
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => {
                const hasImg = !!it.src;
                // Priorità di fetch: più vicino al centro -> "high"
                ...
                return (
                  <div
                    ...
                    {...(hasImg ? { "data-src": it.src } : {})}
                    {...(it.alt ? { "data-alt": it.alt } : {})}
                    data-offset-x={it.x}
                    data-offset-y={it.y}
                  ...
                  >
                    <div
                      className={`item__image absolute block overflow-hidden transition-transform duration-300 ${
                        hasImg
                          ? "cursor-pointer bg-[#E9D9B3]" // crema calda come placeholder
                          : "pointer-events-none bg-transparent"
                      }`}
                      role={hasImg ? "button" : undefined}
                      tabIndex={hasImg ? 0 : -1}
                      aria-label={hasImg ? it.alt || "Open image" : undefined}
                      onClick={(e) => {
                        if (!hasImg) return;
                    ...
                    >
                      {hasImg ? (
                        <img
                          // Lazy “vera”: src fittizia, quella reale sta in data-src e viene messa dall’IO
                          src={PLACEHOLDER_1x1}
                          data-src={it.src}
                          alt={it.alt}
                          draggable={false}
                          ...
                          onLoad={(e) => {
                            // quando carica quella reale, togli blur
